...
const futhark_binary_format_version: number = 2;
...
class FutharkReader {
  ...
  private read_byte(): number {
    ...
    if (b === undefined) {
      throw new FutharkError(
        'Cannot read byte: buffer is empty',
        { operation: 'read_byte' }
      );
    }
    this.seek(1);
   ...
   */
  private read_i64(): bigint {
    if (this.buf.length < 8) {
      throw new FutharkError(
        'Cannot read i64: buffer has less than 8 bytes',
        { operation: 'read_i64', availableBytes: this.buf.length }
      );
    }
    ...
    if (val === undefined) {
      throw new FutharkError(
        'Cannot read i64: unexpected undefined value',
        { operation: 'read_i64' }
      );
    }
    this.seek(8);
   ...
   */
  read_value(expected_type?: string): FutharkValue {
    // Check for sufficient data
    if (this.buf.length === 0) {
      throw new FutharkError(
        'Cannot read value: buffer is empty',
        { operation: 'read_value', expectedType: expected_type }
      );
    }
    ...
    while (off < this.buf.length) {
      ...
      if (byte === undefined || !this.is_whitespace(byte)) break;
      off++;
    }
    this.seek(off);
    ...
    if (this.buf.length < 7) {
      throw new FutharkError(
        `Invalid binary data: expected at least 7 bytes for header, got ${this.buf.length}`,
        { operation: 'read_value', bufferLength: this.buf.length, expectedType: expected_type }
      );
    }
    ...
    if (magic !== this.byte_val('b')) {
      throw new FutharkError(
        `Invalid binary format: expected magic byte 'b' (0x62), got 0x${magic.toString(16)}`,
        { operation: 'read_value', gotMagic: magic, expectedType: expected_type }
      );
    }
    ...
    if (version !== futhark_binary_format_version) {
      throw new FutharkError(
        `Unsupported binary format version: expected ${futhark_binary_format_version}, got ${version}`,
        { operation: 'read_value', gotVersion: version, supportedVersion: futhark_binary_format_version, expectedType: expected_type }
      );
    }
    ...
    if (!(type in primInfos)) {
      throw new FutharkError(
        `Unknown primitive type in binary data: '${type}'`,
        { operation: 'read_value', unknownType: type, expectedType: expected_type, supportedTypes: Object.keys(primInfos) }
      );
    }
    this.seek(4);
    ...
    if (expected_type !== undefined) {
      if (rank === 0 && expected_type !== type) {
        throw new FutharkError(
          `Type mismatch: read scalar type '${type}', expected '${expected_type}'`,
          { operation: 'read_value', readType: type, expectedType: expected_type, readRank: rank }
        );
      }
      if (rank > 0) {
        let expected_rank = 0;
        ...
        if (rank !== expected_rank || type !== rem_type) {
          ...
          throw new FutharkError(
            `Type mismatch: read '${readTypeStr}', expected '${expected_type}'`,
            { operation: 'read_value', readType: readTypeStr, expectedType: expected_type, readRank: rank, expectedRank: expected_rank }
          );
        }
      }
    }
    ...
    if (this.buf.length < shapeBytes) {
      throw new FutharkError(
        `Truncated binary data: expected ${shapeBytes} bytes for shape (rank ${rank}), got ${this.buf.length}`,
        { operation: 'read_value', expectedBytes: shapeBytes, availableBytes: this.buf.length, rank }
      );
    }
   ...
   */
  private read_array(type: string, shape: bigint[]): FutharkArrayData {
    const type_info = primInfos[type];
    if (!type_info) {
      throw new FutharkError(
        `Unknown primitive type: '${type}'`,
        { operation: 'read_array', type, supportedTypes: Object.keys(primInfos) }
      );
    }
    ...
    if (this.buf.length < required_bytes) {
      throw new FutharkError(
        `Truncated binary data: expected ${required_bytes} bytes for array data, got ${this.buf.length}`,
        { operation: 'read_array', type, shape: shape.map(Number), expectedBytes: required_bytes, availableBytes: this.buf.length }
      );
    }
   ...
   */
  private is_whitespace(b: number): boolean {
   ...
   */
  private byte_val(c: string): number { return c.charCodeAt(0); }
}
...
class FutharkWriter {
   ...
   */
  encode_value(val: FutharkValue, type: string): Uint8Array {
    let elem_type: string;
    let rank: number = 0;
    let flat_len: number = 0;
    ...
    if (type in primInfos) {
      ...
      if (Array.isArray(val) && val.length === 2 && Array.isArray(val[1])) {
        throw new FutharkError(
          `Type mismatch: expected scalar type '${type}', got array data`,
          { operation: 'encode_value', expectedType: type, gotType: 'array' }
        );
      }
    }
    else {
      ...
      if (!(elem_type in primInfos)) {
        throw new FutharkError(
          `Unknown element type '${elem_type}' in type '${type}'`,
          { operation: 'encode_value', type, elementType: elem_type, supportedTypes: Object.keys(primInfos) }
        );
      }
      // Validate that val is array data
      if (!Array.isArray(val) || val.length !== 2) {
        throw new FutharkError(
          `Type mismatch: expected array data [values, shape] for type '${type}', got ${typeof val}`,
          { operation: 'encode_value', expectedType: type, gotType: typeof val }
        );
      }
      const [, shape] = val as FutharkArrayData;
      rank = shape.length;
    ...
    if (rank === 0) {
      ...
      const scalarVal = val as number | bigint;
      // Create the array based on the type
      if (prim_info.array_type === BigInt64Array) {
        data = new BigInt64Array([scalarVal as bigint]);
      } else if (prim_info.array_type === BigUint64Array) {
        data = new BigUint64Array([scalarVal as bigint]);
      } else if (prim_info.array_type === Float16Array) {
        data = new Float16Array([scalarVal as number]);
      } else if (prim_info.array_type === Float32Array) {
        data = new Float32Array([scalarVal as number]);
      } else if (prim_info.array_type === Float64Array) {
        data = new Float64Array([scalarVal as number]);
      } else if (prim_info.array_type === Int8Array) {
        data = new Int8Array([scalarVal as number]);
      } else if (prim_info.array_type === Uint8Array) {
        data = new Uint8Array([scalarVal as number]);
      } else if (prim_info.array_type === Int16Array) {
        data = new Int16Array([scalarVal as number]);
      } else if (prim_info.array_type === Uint16Array) {
        data = new Uint16Array([scalarVal as number]);
      } else if (prim_info.array_type === Int32Array) {
        data = new Int32Array([scalarVal as number]);
      } else {
        data = new Uint32Array([scalarVal as number]);
      }
      shape = [];
    }
    else {
      const [d, s] = val as FutharkArrayData;
      data = d;
      shape = s;
    }
