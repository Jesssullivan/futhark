...
/**
 * Stored variable containing a value and its type.
 */
interface StoredVariable {
  /** The stored value */
  val: FutharkScalar | FutharkArray;
  /** The Futhark type string */
  typ: string;
}
...
interface ServerResponse {
  ...
  text?: string;
  /** Base64 encoded data (for store command) */
  data?: string;
  /** Type strings (for store command) */
  types?: string[];
}

/**
 * Command message format from WebSocket.
 */
 ...
 */
type CommandHandler = (...args: string[]) => Promise<string | { data: string; types: string[] }>;

/**
 * Declares the global window interface extensions.
 */
interface FutharkWindow extends Window {
  fut: FutharkModule;
  server: BrowserServer;
}

/** Global FutharkModule class from generated code */
declare function Module(): Promise<FutharkEmscriptenModule>;

/** Generated FutharkModule wrapper class */
declare const FutharkModuleClass: {
  new(): FutharkModule & { init(m: FutharkEmscriptenModule): Promise<void> };
};
...
class BrowserServer {
  ...
  private fut: FutharkModule;
  /** Stored variables by name */
  private vars: Record<string, StoredVariable>;
   ...
   */
  constructor(fut: FutharkModule, _port?: number) {
    this.fut = fut;
    this.vars = {};
      ...
      try {
        if (!msg.cmd || typeof msg.cmd !== 'string') {
          throw new FutharkError(
            'Invalid command message: missing or invalid "cmd" field',
            { receivedMessage: msg }
          );
        }
        ...
        if (!(msg.cmd in this.commands)) {
          ...
          throw new FutharkError(
            `Unknown command: '${msg.cmd}'`,
            { command: msg.cmd, availableCommands: available }
          );
        }
  ...
  private get_entry_point(entry: string): (...args: (FutharkScalar | FutharkArray)[]) => Promise<(FutharkScalar | FutharkArray)[]> {
    ...
    throw new FutharkError(
      `Unknown entry point: '${entry}'`,
      { operation: 'get_entry_point', entryPoint: entry, availableEntryPoints: available }
    );
  }
  ...
  private get_manifest_entry_point(entry: string): ManifestEntryPoint {
    ...
    throw new FutharkError(
      `Unknown entry point: '${entry}'`,
      { operation: 'get_manifest_entry_point', entryPoint: entry, availableEntryPoints: available }
    );
  }
  ...
  private get_manifest_type(type: string): ManifestTypeInfo {
    ...
    throw new FutharkError(
      `Unknown type: '${type}'`,
      { operation: 'get_manifest_type', type, availableTypes: available }
    );
  }
   ...
   */
  private check_var(name: string): void {
    if (!(name in this.vars)) {
      const available = Object.keys(this.vars);
      throw new FutharkError(
        `Unknown variable: '${name}'`,
        { operation: 'check_var', variable: name, availableVariables: available }
      );
    }
  }
   ...
   */
  private set_var(name: string, val: FutharkScalar | FutharkArray, typ: string): void {
   ...
   * @param name - Variable name
   */
  private delete_var(name: string): void {
   ...
   */
  private async cmd_entry_points(): Promise<string> {
   ...
   */
  private async cmd_inputs(entry: string): Promise<string> {
   ...
   */
  private async cmd_outputs(entry: string): Promise<string> {
    const entry_info = this.get_manifest_entry_point(entry);
   ...
   */
  private async cmd_restore(file: string, ...varsAndTypes: string[]): Promise<string> {
    // Validate arguments
    if (!file) {
      throw new FutharkError(
        'restore: file URL is required',
        { operation: 'restore' }
      );
    }
    ...
    if (varsAndTypes.length === 0 || varsAndTypes.length % 2 !== 0) {
      throw new FutharkError(
        `restore: expected even number of arguments (variable name, type pairs), got ${varsAndTypes.length}`,
        { operation: 'restore', argumentCount: varsAndTypes.length }
      );
    }
    ...
    try {
      ...
      throw new FutharkError(
        `restore: failed to fetch file '${file}': ${fetchError instanceof Error ? fetchError.message : String(fetchError)}`,
        { operation: 'restore', file, error: String(fetchError) }
      );
    }

    if (!response.ok) {
      throw new FutharkError(
        `restore: failed to fetch file '${file}': ${response.status} ${response.statusText}`,
        { operation: 'restore', file, status: response.status, statusText: response.statusText }
      );
    }
    ...
    for (let i = 0; i < varsAndTypes.length; i += 2) {
      ...
      if (!name || !type) {
        throw new FutharkError(
          `restore: invalid variable/type pair at index ${i}`,
          { operation: 'restore', index: i, name, type }
        );
      }
      ...
      try {
        ...
        let val: FutharkScalar | FutharkArray;
        if (type in this.fut.manifest.types) {
          const type_info = this.get_manifest_type(type);
          futhark_assert(
            type_info.kind === "array",
            `restore: type '${type}' is not an array type`,
            { operation: 'restore', variable: name, type, typeKind: type_info.kind }
          );
          ...
          // Scalar.
          val = raw_val as FutharkScalar;
        }

        this.set_var(name, val, type);
      } catch (readError) {
        throw new FutharkError(
          `restore: failed to read variable '${name}' of type '${type}': ${readError instanceof Error ? readError.message : String(readError)}`,
          { operation: 'restore', variable: name, type, error: String(readError) }
        );
      }
    }
   ...
   */
  private async cmd_store(...vars: string[]): Promise<{ data: string; types: string[] }> {
    let data = "";
    const types: string[] = [];
    for (const name of vars) {
      const {val, typ} = this.get_var(name);

      let to_write: FutharkValue;
      if (typ in this.fut.manifest.types) {
        const type_info = this.get_manifest_type(typ);
        futhark_assert(type_info.kind === "array");

        const values = await (val as FutharkArray).values();
        const shape = (val as FutharkArray).get_shape();
        to_write = [values, Array.from(shape)] as FutharkArrayData;
      }
      else {
        // Scalar.
        to_write = val as FutharkScalar;
      }
   ...
   * @returns Empty string on success
   */
  private async cmd_free(name: string): Promise<string> {
   ...
   */
  private async cmd_call(entry: string, ...outsAndIns: string[]): Promise<string> {
    const entry_info = this.get_manifest_entry_point(entry);
   ...
   * @returns Empty string on success
   */
  private async cmd_clear(): Promise<string> {
   ...
   */
  private async cmd_report(): Promise<string> {
   ...
   * @returns Empty string on success
   */
  private async cmd_pause_profiling(): Promise<string> {
   ...
   * @returns Empty string on success
   */
  private async cmd_unpause_profiling(): Promise<string> {
...
async function runServer(): Promise<void> {
  ...
  const win = window as unknown as FutharkWindow;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  win.fut = new (FutharkModuleClass as any)();
  await (win.fut as FutharkModule & { init(m: FutharkEmscriptenModule): Promise<void> }).init(m as FutharkEmscriptenModule);
